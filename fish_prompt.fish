# name: 23dfish
#
# 23dfish is a Powerline-style, Git-aware fish theme optimized for awesome.
#
...
function __23dfish_prompt_os
    if test "$theme_show_os" = 'yes'
        ...
        switch (uname)
            case Android
                printf $android_glyph
            case Darwin
                printf $apple_glyph
            case FreeBSD
                printf $freebsd_glyph
            case Linux
                switch (cat /etc/os-release | grep "^ID=")
                    case "*arch*"
                        printf $linux_arch_glyph
                    case "*centos*"
                        printf $linux_centos_glyph
                    case "*debian*"
                        printf $linux_debian_glyph
                    case "*raspbian*"
                        printf $linux_raspbian_glyph
                    case "*elementary*"
                        printf $linux_elementary_glyph
                    case "*fedora*"
                        printf $linux_fedora_glyph
                    case "*gentoo*"
                        printf $linux_gentoo_glyph
                    case "*mageia*"
                        printf $linux_mageia_glyph
                    case "*mint*"
                        printf $linux_mint_glyph
                    case "*nixos*"
                        printf $linux_nixos_glyph
                    case "*manjaro*"
                        printf $linux_manjaro_glyph
                    case "*devuan*"
                        printf $linux_devuan_glyph
                    case "*alpine*"
                        printf $linux_alpine_glyph
                    case "*aosc*"
                        printf $linux_aosc_glyph
                    case "*opensuse*|*tumbleweed*"
                        printf $linux_opensuse_glyph
                    case "*sabayon*"
                        printf $linux_sabayon_glyph
                    case "*slackware*"
                        printf $linux_slackware_glyph
                    case "*ubuntu*"
                        printf $linux_ubuntu_glyph
                    case "*"
                        printf $linux_glyph
                end
            case SunOS
...
function __23dfish_basename -d 'basically basename, but faster'
    string replace -r '^.*/' '' -- $argv
end

function __23dfish_dirname -d 'basically dirname, but faster'
    string replace -r '/[^/]+/?$' '' -- $argv
end

function __23dfish_pwd -d 'Get a normalized $PWD'
...
function __23dfish_escape_regex -a str -d 'A backwards-compatible `string escape --style=regex` implementation'
    string escape --style=regex "$str" 2>/dev/null
    or echo "$str"
end

function __23dfish_git_branch -S -d 'Get the current git branch (or commitish)'
...
function __23dfish_hg_branch -S -d 'Get the current hg branch'
    set -l branch (command hg branch 2>/dev/null)
    set -l book (command hg book | command grep \* | cut -d\  -f3)
    echo "$branch_glyph $branch @ $book"
end

function __23dfish_pretty_parent -S -a child_dir -d 'Print a parent directory, shortened to fit the prompt'
    set -q fish_prompt_pwd_dir_length
    or set -l fish_prompt_pwd_dir_length 0

    # Replace $HOME with ~
    set -l real_home ~
    set -l parent_dir (string replace -r '^'(__23dfish_escape_regex "$real_home")'($|/)' '~$1' (__23dfish_dirname $child_dir))
...
function __23dfish_ignore_vcs_dir -a real_pwd -d 'Check whether the current directory should be ignored as a VCS segment'
...
function __23dfish_git_project_dir -S -a real_pwd -d 'Print the current git project base directory'
    [ "$theme_display_git" = 'no' ]
    and return

    set -q theme_vcs_ignore_paths
    and [ (__23dfish_ignore_vcs_dir $real_pwd) ]
    and return
    ...
    set -l project_dir (__23dfish_dirname $git_dir)
...
function __23dfish_hg_project_dir -S -a real_pwd -d 'Print the current hg project base directory'
...
function __23dfish_project_pwd -S -a project_root_dir -a real_pwd -d 'Print the working directory relative to project root'
    set -q theme_project_dir_length
    or set -l theme_project_dir_length 0

    set -l project_dir (string replace -r '^'(__23dfish_escape_regex "$project_root_dir")'($|/)' '' $real_pwd)
...
function __23dfish_git_ahead -S -d 'Print the ahead/behind state for the current branch'
    if [ "$theme_display_git_ahead_verbose" = 'yes' ]
        __23dfish_git_ahead_verbose
        return
    end
...
function __23dfish_git_ahead_verbose -S -d 'Print a more verbose ahead/behind state for the current branch'
...
function __23dfish_git_dirty_verbose -S -d 'Print a more verbose dirty state for the current working tree'
...
function __23dfish_git_stashed -S -d 'Print the stashed state for the current branch'
...
function __23dfish_start_segment -S -d 'Start a prompt segment'
    set -l bg $argv[1]
    set -e argv[1]
    set -l fg $argv[1]
    set -e argv[1]
...
function __23dfish_path_segment -S -a segment_dir -d 'Display a shortened form of a directory'
    ...
    __23dfish_start_segment $segment_color
    ...
    if [ "$theme_color_scheme" != '23d' ]
        ...
        set_color $segment_basename_color
    end
    echo -ns $directory ' '
end

function __23dfish_finish_segments -S -d 'Close open prompt segments'
    if [ -n "$__23dfish_current_bg" ]
        set_color normal
        set_color $__23dfish_current_bg
        echo -ns $right_black_arrow_glyph ' '
    end
...
function __23dfish_prompt_status -S -a last_status -d 'Display flags for a non-zero exit status, root user, and background jobs'
    set -l nonzero
    set -l superuser
    set -l bg_jobs
    ...
    if [ "$nonzero" -o "$superuser" -o "$bg_jobs" ]
        __23dfish_start_segment $color_initial_segment_exit
        if [ "$nonzero" ]
            set_color normal
...
function __23dfish_prompt_vagrant -S -d 'Display Vagrant status'
    ...
    for file in .vagrant/machines/*/*/id
        ...
        if [ -n "$id" ]
            switch "$file"
                case '*/virtualbox/id'
                    __23dfish_prompt_vagrant_vbox $id
                case '*/vmware_fusion/id'
                    __23dfish_prompt_vagrant_vmware $id
                case '*/parallels/id'
                    __23dfish_prompt_vagrant_parallels $id
            end
        end
    end
end

function __23dfish_prompt_vagrant_vbox -S -a id -d 'Display VirtualBox Vagrant status'
    ...
    __23dfish_start_segment $color_vagrant
    echo -ns $vagrant_status ' '
end

function __23dfish_prompt_vagrant_vmware -S -a id -d 'Display VMWare Vagrant status'
    set -l vagrant_status
    if [ (pgrep -f "$id") ]
        set vagrant_status "$vagrant_status$vagrant_running_glyph"
    else
...
function __23dfish_prompt_vagrant_parallels -S -d 'Display Parallels Vagrant status'
    set -l vagrant_status
...
function __23dfish_prompt_docker -S -d 'Display Docker machine name'
...
function __23dfish_k8s_context -S -d 'Get the current k8s context'
...
function __23dfish_k8s_namespace -S -d 'Get the current k8s namespace'
    kubectl config view --minify --output "jsonpath={..namespace}"
end

function __23dfish_prompt_k8s_context -S -d 'Show current Kubernetes context'
    [ "$theme_display_k8s_context" = 'yes' ]
    or return

    set -l context (__23dfish_k8s_context)
    or return

    [ "$theme_display_k8s_namespace" = 'yes' ]
    and set -l namespace (__23dfish_k8s_namespace)
    ...
    __23dfish_start_segment $color_k8s
    echo -ns $segment " "
end
...
if not type -q prompt_hostname
    if not set -q __23dfish_prompt_hostname
        set -g __23dfish_prompt_hostname (hostname | string replace -r '\..*' '')
    end

    function prompt_hostname
        echo $__23dfish_prompt_hostname
    end
end

function __23dfish_prompt_user -S -d 'Display current user and hostname'
    ...
    if set -q display_user
        if [ "$SSH_CLIENT" ]
            ...
            set_color $color_initial_segment_jobs
            echo -ns ' ' $ssh_glyph ' '
        end
    ...
    if set -q display_hostname
        if set -q display_user
            ...
            if [ "$theme_color_scheme" != '23d' ]
                ...
                set_color $color_colon[2]
                echo -ns ':'
                set_color $color_hostname[2..-1]
                echo -ns (prompt_hostname)
            end
        else
...
function __23dfish_rvm_parse_ruby -S -a ruby_string -a scope -d 'Parse RVM Ruby string'
    # Function arguments:
...
function __23dfish_rvm_info -S -d 'Current Ruby information from RVM'
    ...
    __23dfish_rvm_parse_ruby $__rvm_default_ruby default
    __23dfish_rvm_parse_ruby $__rvm_current_ruby current
...
function __23dfish_prompt_rubies -S -d 'Display current Ruby information'
    [ "$theme_display_ruby" = 'no' ]
    and return

    set -l ruby_version
    if type -fq rvm-prompt
        set ruby_version (__23dfish_rvm_info)
    ...
    __23dfish_start_segment $color_rvm
    echo -ns $ruby_glyph $ruby_version ' '
end

function __23dfish_virtualenv_python_version -S -d 'Get current Python version'
...
function __23dfish_prompt_virtualfish -S -d "Display current Python virtual environment (only for virtualfish, virtualenv's activate.fish changes prompt by itself) or conda environment."
    [ "$theme_display_virtualenv" = 'no' -o -z "$VIRTUAL_ENV" -a -z "$CONDA_DEFAULT_ENV" ]
    and return

    set -l version_glyph (__23dfish_virtualenv_python_version)

    if [ "$version_glyph" ]
        __23dfish_start_segment $color_virtualfish
...
function __23dfish_prompt_virtualgo -S -d 'Display current Go virtual environment'
    [ "$theme_display_virtualgo" = 'no' -o -z "$VIRTUALGO" ]
    and return

    __23dfish_start_segment $color_virtualgo
    echo -ns $go_glyph ' ' (basename "$VIRTUALGO") ' '
    set_color normal
end

function __23dfish_prompt_desk -S -d 'Display current desk environment'
    [ "$theme_display_desk" = 'no' -o -z "$DESK_ENV" ]
    and return

    __23dfish_start_segment $color_desk
    echo -ns $desk_glyph ' ' (basename  -a -s ".fish" "$DESK_ENV") ' '
    set_color normal
end

function __23dfish_prompt_nvm -S -d 'Display current node version through NVM'
    ...
    __23dfish_start_segment $color_nvm
    echo -ns $node_glyph $node_version ' '
    set_color normal
end

function __23dfish_prompt_nix -S -d 'Display current nix environment'
    [ "$theme_display_nix" = 'no' -o -z "$IN_NIX_SHELL" ]
    and return

    __23dfish_start_segment $color_nix
...
function __23dfish_prompt_hg -S -a hg_root_dir -a real_pwd -d 'Display the actual hg state'
    ...
    __23dfish_path_segment $hg_root_dir

    __23dfish_start_segment $flag_colors
    echo -ns $hg_glyph ' '

    __23dfish_start_segment $flag_colors
    echo -ns (__23dfish_hg_branch) $flags ' '
    set_color normal

    set -l project_pwd (__23dfish_project_pwd $hg_root_dir $real_pwd)
    if [ "$project_pwd" ]
        if [ -w "$real_pwd" ]
            __23dfish_start_segment $color_path
        else
            __23dfish_start_segment $color_path_nowrite
        end
...
function __23dfish_prompt_git -S -a git_root_dir -a real_pwd -d 'Display the actual git state'
    set -l dirty ''
    if [ "$theme_display_git_dirty" != 'no' ]
        ...
        if [ "$show_dirty" != 'false' ]
            ...
            if [ "$dirty" -a "$theme_display_git_dirty_verbose" = 'yes' ]
                set dirty "$dirty"(__23dfish_git_dirty_verbose)
            end
        end
    end

    set -l staged (command git diff --cached --no-ext-diff --quiet --exit-code 2>/dev/null; or echo -n "$git_staged_glyph")
    set -l stashed (__23dfish_git_stashed)
    set -l ahead (__23dfish_git_ahead)
    ...
    __23dfish_path_segment $git_root_dir

    __23dfish_start_segment $flag_colors
    echo -ns (__23dfish_git_branch) $flags ' '
    set_color normal

    if [ "$theme_git_worktree_support" != 'yes' ]
        set -l project_pwd (__23dfish_project_pwd $git_root_dir $real_pwd)
        if [ "$project_pwd" ]
            if [ -w "$real_pwd" ]
                __23dfish_start_segment $color_path
            else
    ...
    if [ "$project_pwd" -o "$work_dir" ]
        ...
        __23dfish_start_segment $colors

        # handle work_dir != project dir
        if [ "$work_dir" ]
            set -l work_parent (__23dfish_dirname $work_dir)
            if [ "$work_parent" ]
                echo -n "$work_parent/"
            end
            ...
            echo -n (__23dfish_basename $work_dir)
...
function __23dfish_prompt_dir -S -a real_pwd -d 'Display a shortened form of the current directory'
    __23dfish_path_segment "$real_pwd"
end

function __23dfish_prompt_char -d "Terminate with a nice prompt char"
  set -q theme_prompt_char
    or set -U theme_prompt_char '▶︎'
  echo -n -s $normal $theme_prompt_char ' '
end
...
function fish_prompt -d '23dfish, a fish theme optimized for awesome'
    ...
    set -l __23dfish_current_bg

    # Status flags and input mode
    __23dfish_prompt_status $last_status

    # OS
    __23dfish_prompt_os

    # User / hostname info
    __23dfish_prompt_user

    # Containers and VMs
    __23dfish_prompt_vagrant
    __23dfish_prompt_docker
    __23dfish_prompt_k8s_context

    # Virtual environments
    __23dfish_prompt_nix
    __23dfish_prompt_desk
    __23dfish_prompt_rubies
    __23dfish_prompt_virtualfish
    __23dfish_prompt_virtualgo
    __23dfish_prompt_nvm

    set -l real_pwd (__23dfish_pwd)

    # VCS
    set -l git_root_dir (__23dfish_git_project_dir $real_pwd)
    set -l hg_root_dir (__23dfish_hg_project_dir $real_pwd)

    if [ "$git_root_dir" -a "$hg_root_dir" ]
        # only show the closest parent
        switch $git_root_dir
            case $hg_root_dir\*
                __23dfish_prompt_git $git_root_dir $real_pwd
            case \*
                __23dfish_prompt_hg $hg_root_dir $real_pwd
        end
        ...
        __23dfish_prompt_dir $real_pwd
    end

    __23dfish_finish_segments
    if [ "$theme_color_scheme" = '23d' ]
        __23dfish_prompt_char
    end
end
