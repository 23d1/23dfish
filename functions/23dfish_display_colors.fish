function 23dfish_display_colors -a color_scheme -d 'Print example prompt color schemes'

  set -l color_schemes 23d default light \
  ...
  switch "$color_scheme"
    case '--all'
      for scheme in $color_schemes
        echo
        echo "$scheme:"
        23dfish_display_colors $scheme
      end
      return

    case $color_schemes
      __23dfish_colors $color_scheme

    case ''
      if type -q 23dfish_colors
        if [ -n "$theme_color_scheme" ]
          echo "$theme_color_scheme (with 23dfish_colors overrides):"
        else
          echo 'custom (via 23dfish_colors):'
        end
    ...
    case '*'
      echo 'usage: 23dfish_display_colors [--all] [color_scheme]'
      return
  end
  ...
  set_color -b $color_path_basename
  echo -ns basename ' '
  __23dfish_start_segment $color_repo
  ...
  set_color -b $color_path_basename
  echo -ns basename ' '
  __23dfish_start_segment $color_repo_dirty
  ...
  set_color -b $color_path_basename
  echo -ns basename ' '
  __23dfish_start_segment $color_repo_staged
  echo -n "$detached_glyph repo_staged $git_staged_glyph "
  __23dfish_finish_segments
  echo

  __23dfish_start_segment $color_vi_mode_default
  echo -ns vi_mode_default ' '
  __23dfish_finish_segments
  __23dfish_start_segment $color_vi_mode_insert
  echo -ns vi_mode_insert ' '
  __23dfish_finish_segments
  __23dfish_start_segment $color_vi_mode_visual
